...
def solve_gamma(A_re, A_im, Z_re, Z_im, M, lambda_value):
    ...
    return solve_nonnegative_qp(H, c)


def optimal_lambda(A_re, A_im, Z_re, Z_im, M, data_used, induct_used,
                   log_lambda_0, cv_type, boundary_fallback='LC'):
    """Select the Tikhonov regularization parameter.

    The objective is evaluated on a logarithmic grid over
    1e-7 <= lambda <= 1, followed by local refinement of an interior minimum.
    Boundary solutions are recorded in ``last_diagnostics``. When configured,
    an interior L-curve solution may be used as a fallback.
    """
    objectives = {
        'GCV': param.compute_GCV,
        'mGCV': param.compute_mGCV,
        'rGCV': param.compute_rGCV,
        'LC': param.compute_LC,
        're-im': param.compute_re_im_cv,
        'kf': param.compute_kf_cv,
    }
    if cv_type not in objectives:
        raise ValueError(f"Unknown regularization-selection method: {cv_type}")

    lambda_lower, lambda_upper = 1e-7, 1.0
    log10_grid = np.linspace(np.log10(lambda_lower), np.log10(lambda_upper), 141)
    lambda_grid = 10.0 ** log10_grid

    def search(method):
        objective_fn = objectives[method]

        def scalar_objective(log_lambda):
            try:
                value = objective_fn(
                    float(log_lambda), A_re, A_im, Z_re, Z_im, M,
                    data_used, induct_used
                )
            except Exception:
                return np.inf
            if value is None:
        ...
        scores = np.asarray([scalar_objective(np.log(lam)) for lam in lambda_grid])
        finite = np.isfinite(scores)
        if not finite.any():
            raise RuntimeError(f"{method} regularization optimization failed: no finite objective values")
        finite_indices = np.flatnonzero(finite)
        best_index = int(finite_indices[np.argmin(scores[finite])])
        boundary_hit = best_index in (0, len(lambda_grid) - 1)
        lam = float(lambda_grid[best_index])
        score = float(scores[best_index])
        refine_evals = 0

        if not boundary_hit:
            left = float(np.log(lambda_grid[best_index - 1]))
            right = float(np.log(lambda_grid[best_index + 1]))
            ...
            if result.success and np.isfinite(result.fun):
                lam = float(np.exp(float(result.x)))
                score = float(result.fun)
                refine_evals = int(result.nfev)
        ...
        return {
            ...
            'refinement_evaluations': refine_evals,
        }

    primary = search(cv_type)
    selected = primary
    fallback = None

    if primary['boundary_hit'] and boundary_fallback and cv_type != boundary_fallback:
        fallback = search(boundary_fallback)
        if not fallback['boundary_hit']:
            selected = fallback

    fallback_used = selected is not primary
    lambda_value = float(selected['lambda_value'])

    print(f"{cv_type} regularization selection")
    print(f"  requested criterion minimum: {primary['lambda_value']:.10g}")
    print(f"  requested criterion value:   {primary['objective_value']:.10g}")
    if primary['boundary_hit']:
        side = 'lower' if primary['best_grid_index'] == 0 else 'upper'
        print(f"  WARNING: {cv_type} reached the {side} search boundary.")
    if fallback_used:
        print(
            f"  no interior {cv_type} optimum was found; using interior "
            f"{selected['method']} fallback: lambda={lambda_value:.10g}"
        )
    else:
        print(f"  selected lambda: {lambda_value:.10g}")

    optimal_lambda.last_diagnostics = {
        'requested_method': cv_type,
        'effective_method': selected['method'],
        'fallback_used': bool(fallback_used),
        'boundary_hit': bool(primary['boundary_hit']),
...
optimal_lambda.last_diagnostics = None
